...
inline
TVector<T>::TVector()
	:	m_space( defaultSpace )
{
	m_pFirst = (T*) operator new( defaultSpace * sizeof( T ) );
	m_pAfterLast = m_pFirst;
}
...
{
	size_t s = size();
	size_t newSpace = m_space << 1;
	while ( newSpace < (n + m_space) )
	{
		newSpace << 1;
	}
	T* pBuffer = (T*)operator new( bytes(newSpace) );
