...
The CkyMunge sample filter is no longer included with the Platform SDK.
This is unfortunate, since it's the only non-trivial sample of an ISAPI
send-raw-data filter that modifies the outgoing stream.

CkyMunge assumes that tokens in the response body can be split across
multiple sends (WriteClients) and buffers what it needs to in order
to correctly tokenize the HTML stream. However, it makes the simplifying
assumption that the response headers will not be split in the middle of
a line.

In general, it is much too expensive for CkyMunge to buffer an entire
response before writing out the modified response, especially if the
response is long or if there are multiple simultaneous requests being
handled by the server. CkyMunge removes the response Content-Length header
and destroys a "Connection: Keep-alive" header in the request, if it's
present. This causes IIS to close the connection after sending the last of
the response data. HTTP Keep-Alives are a good thing. It would be better if
CkyMunge left the request Connection header alone and replaced the response
Content-Length header (if present) with "Transfer-Encoding: chunked", and
rewrote the chunks as appropriate. All modern browsers understand chunking.
